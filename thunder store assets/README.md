...
3. 在 `BepInEx/plugins` 文件夹中创建一个名为 `Silken Impact`（或任意其他名称）的文件夹。

   > [!note]
   >
   > 安装 BepInEx 后需要启动一次游戏，plugins 文件夹才会显示。

4. 将包含的 `.dll` 文件和 `Assets` 文件夹放入 `Silken Impact` 文件夹中。



## 发布页面

主网站：[Nexus](https://www.nexusmods.com/hollowknightsilksong/mods/686?tab=posts&comment_id=161592433)

也可在 [Thunderstore](https://thunderstore.io/c/hollow-knight-silksong/p/Kokomi/Silken_Impact/)、[3DM Mod站](https://mod.3dmgame.com/mod/245006) 下载

> 欢迎在 Nexus 上留言反馈和报告漏洞。如果你喜欢这个 Mod，请给予评分支持 ❤
