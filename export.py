...
def make_thunderstore_output(version: str):
    ...
    try:
        ...
        for entry in os.listdir(BEPINEX_FOLDER):
            ...
            dst = os.path.join(plugins_dir, entry)
            if os.path.isdir(src):
                shutil.copytree(src, dst)
            else:
                shutil.copy2(src, dst)
...
def check_versions(release_version) -> bool:
    ...
    if log_latest_version != release_version:
        passed = False
        print(f"Warning: The version in {CHANGE_LOG_PATH} ({log_latest_version}) does not match the specified version ({release_version}).")
        return passed
    ...
    if project_version != release_version:
        ...
        op = input("Do you want to build the project again before bundling? (y/n): ").strip().lower()
        if op == "y":
            if build_project():
